...
// Send is wired by hand (not apiFetch) so we can read the structured
// `retryAfterSeconds` field on a 429 and surface a precise countdown.
async function sendMessage(
  ...
  if (!res.ok) {
    ...
    // 503 is the "AI offline" case — surface as a one-line toast, not the
    // generic API modal.
    if (res.status !== 503) {
